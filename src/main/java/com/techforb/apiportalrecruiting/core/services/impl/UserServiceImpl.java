...
public class UserServiceImpl implements UserService {
  ...
  private final RestTemplate restTemplate;
    ...
    if (response == null || !response.containsKey(ID_TOKEN)) {
		...
		Object providerUserInfo = response.get("providerUserInfo");
		if (providerUserInfo instanceof List<?> list && !list.isEmpty()) {
			Object first = list.get(0);
			if (first instanceof Map<?, ?> map) {
				Object photoUrl = map.get("photoUrl");
				if (photoUrl != null) {
					userResponse.setProfileImageUrl(String.valueOf(photoUrl));
				}
			}
		}
