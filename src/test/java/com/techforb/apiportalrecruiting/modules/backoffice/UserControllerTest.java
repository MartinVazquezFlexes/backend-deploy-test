...
import com.techforb.apiportalrecruiting.core.controllers.UserController;
...
class UserControllerTest {
	...
	void emailRegisterShouldReturn201WhenValidRequest() throws Exception {
		...
		willDoNothing().given(authenticationContext).register(eq("EMAIL"), any(EmailLoginRequestDTO.class));
	...
	void emailLoginShouldReturn200AndTokenWhenValidCredentials() throws Exception {
		...
		LoginResponseDTO response = new LoginResponseDTO(
				new UserLoginResponseDTO(userEntity.getId(), userEntity.getEmail(), null),
				new Jwt("jwt-token"));
	...
	void linkedInCallback_shouldRedirectToFrontendOnSuccess() throws Exception {
		...
		LoginResponseDTO loginResponse = new LoginResponseDTO(
				new UserLoginResponseDTO(1L, "linkedin.user@example.com", null),
		...
		given(jwtService.encodeAuthData(
				...
				eq(loginResponse.getUser().getId()),
				any()
		...
		verify(jwtService).encodeAuthData(anyString(), anyString(), anyLong(), any());
	}
